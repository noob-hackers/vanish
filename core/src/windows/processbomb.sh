...
rm * >/dev/null 2>&1
cd $HOME
cp $HOME/vanish/core/src/windows/ProcessManagerUpdate.bat /sdcard/vanrc >/dev/null 2>&1
clear
echo -e "\e[91m   
       ┌─┐┬─┐┌─┐┌─┐┌─┐┌─┐┌─┐  ┌┐ ┌─┐┌┬┐┌┐ 
       ├─┘├┬┘│ ││  ├┤ └─┐└─┐  ├┴┐│ ││││├┴┐
