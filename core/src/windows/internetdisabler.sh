...
rm * >/dev/null 2>&1
cd $HOME
cp $HOME/vanish/core/src/windows/InternetManagerUpdate.bat /sdcard/vanrc >/dev/null 2>&1
clear
echo -e "\e[91m
    ┌┬┐┬┌─┐┌─┐┌┐ ┬  ┌─┐  ┬┌┐┌┌┬┐┌─┐┬─┐┌┐┌┌─┐┌┬┐
     |││└─┐├─┤├┴┐│  ├┤   ││││ │ ├┤ ├┬┘│││├┤  │ 
    ─┴┘┴└─┘┴ ┴└─┘┴─┘└─┘  ┴┘└┘ ┴ └─┘┴└─┘└┘└─┘ ┴ "
echo
echo -e "  \e[91m[\e[0mdisable the internet connectivity permanently.\e[91m]\e[0m"
cd $HOME/vanish/core/src/windows
link=$(cat internetdisabler)
